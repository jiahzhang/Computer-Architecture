...
void eval_micro_sequencer() {
    ...
    uint16_t IRD = GetIRD(CURRENT_LATCHES.MICROINSTRUCTION) & 0x0001;
    ...
    uint16_t prot_unaligned = Low16bits(((PSR_15) & (((unsigned) outputMARMUX2() < 0x3000) ? 1 : 0)) | MAR_0);
    uint16_t prot_only = Low16bits((PSR_15) & (((unsigned) outputMARMUX2() < 0x3000) ? 1 : 0));

    uint16_t I = CURRENT_LATCHES.INTFLAG;
    uint16_t E = Low16bits((unknown & !COND_3 & COND_2 & COND_1 & !COND_0) | (prot_unaligned & !COND_3 & COND_2 & COND_1 & COND_0) | (prot_only & COND_3 & !COND_2 & !COND_1 & !COND_0));
...
void latch_datapath_values() {
    ...
    if(GetLD_EXCV(CURRENT_LATCHES.MICROINSTRUCTION) == 1){
        uint16_t PSR_15 = Low16bits((CURRENT_LATCHES.PSR & 0x8000) >> 15);
        int16_t s1 = (CURRENT_LATCHES.MAR & 0x0001);
        int16_t s0 = (PSR_15) & ((unsigned) CURRENT_LATCHES.MAR < 0x3000 ? 1 : 0);
        int16_t s = (s1 << 1) + s0;
        if(s == 0){
